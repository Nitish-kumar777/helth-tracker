...
import {
    Flame, Target, Zap, Trophy, TrendingUp, BarChart3,
    CalendarDays, Star, Award, AlertCircle, Sparkles,
    Crown, ArrowUp, ArrowDown, Shield
} from "lucide-react"

// ── Badge config ──────────────────────────────────────────────────────────────
const BADGE_META = {
    BRONZE:        { label: "Bronze",    color: "text-amber-600",   bg: "bg-amber-900/30",    border: "border-amber-700/30",   glow: "shadow-[0_0_12px_rgba(146,64,14,0.4)]"    },
    SILVER:        { label: "Silver",    color: "text-slate-300",   bg: "bg-slate-700/30",    border: "border-slate-500/30",   glow: "shadow-[0_0_12px_rgba(100,116,139,0.4)]"  },
    GOLD:          { label: "Gold",      color: "text-yellow-400",  bg: "bg-yellow-900/25",   border: "border-yellow-600/30",  glow: "shadow-[0_0_16px_rgba(202,138,4,0.5)]"    },
    DIAMOND:       { label: "Diamond",   color: "text-cyan-300",    bg: "bg-cyan-900/25",     border: "border-cyan-500/30",    glow: "shadow-[0_0_20px_rgba(6,182,212,0.5)]"    },
    MILESTONE_7:   { label: "7 Days",    color: "text-violet-300",  bg: "bg-violet-900/25",   border: "border-violet-500/30",  glow: "shadow-[0_0_14px_rgba(124,58,237,0.4)]"  },
    MILESTONE_30:  { label: "30 Days",   color: "text-indigo-300",  bg: "bg-indigo-900/25",   border: "border-indigo-500/30",  glow: "shadow-[0_0_14px_rgba(67,56,202,0.4)]"   },
    MILESTONE_60:  { label: "60 Days",   color: "text-blue-300",    bg: "bg-blue-900/25",     border: "border-blue-500/30",    glow: "shadow-[0_0_14px_rgba(29,78,216,0.4)]"   },
    MILESTONE_90:  { label: "90 Days",   color: "text-sky-300",     bg: "bg-sky-900/25",      border: "border-sky-500/30",     glow: "shadow-[0_0_16px_rgba(2,132,199,0.4)]"   },
    MILESTONE_180: { label: "180 Days",  color: "text-emerald-300", bg: "bg-emerald-900/25",  border: "border-emerald-500/30", glow: "shadow-[0_0_20px_rgba(5,150,105,0.5)]"   },
}
...
function BarChart({ data = [], color = "#818cf8" }) {
    ...
    if (!data.length) return (
        <div className="flex items-center justify-center h-20 text-[12px] text-white/20">
            No data yet
        </div>
    )

    const slice = data.slice(-14)
    const max = Math.max(...slice.map(d => d.accuracy ?? 0), 1)

    return (
        <div ref={containerRef} className="relative flex items-end gap-1 h-20">
            {slice.map((d, i) => {
                const val = d.accuracy ?? 0
                    ...
                    <div
                        ...
                        onMouseEnter={e => {
                            ...
                            setTooltip({
                                ...
                                label: `${d.week !== undefined ? `Wk ${d.week}` : d.month ?? ""} · ${val.toFixed(0)}%`,
                            })
                        }}
            ...
            {/* Floating tooltip — not clipped by overflow */}
            {tooltip && (
                <div
                    className="pointer-events-none absolute z-20 bg-[#1a1a2e] border border-white/[0.1] rounded-lg px-2 py-1 text-[10px] font-mono text-white/70 whitespace-nowrap -translate-x-1/2 -translate-y-full"
...
function AccuracyArc({ pct = 0, color = "#818cf8", size = 120, label }) {
    ...
    const off = half - (Math.min(100, pct) / 100) * half
    // Extra bottom padding so strokeLinecap="round" end-caps aren't clipped
    const vbH = size / 2 + stroke + 4
        ...
        <div className="flex flex-col items-center gap-2">
            <div className="relative">
                <svg
                    ...
                    aria-label={`${label ?? "Accuracy"}: ${pct.toFixed(0)}%`}
                    role="img"
                >
                    <path
                        d={`M ${stroke} ${size / 2} A ${r} ${r} 0 0 1 ${size - stroke} ${size / 2}`}
                ...
                {/* Percentage centred inside the arc bowl */}
...
function StreakBlock({ current = 0, longest = 0 }) {
    ...
    const LEVEL_GLOW = {
        LOW:    "",
        MEDIUM: "drop-shadow-[0_0_8px_rgba(251,146,60,0.6)]",
        HIGH:   "drop-shadow-[0_0_12px_rgba(249,115,22,0.8)]",
        ELITE:  "drop-shadow-[0_0_16px_rgba(239,68,68,1)]",
    }
    const LEVEL_PILL = {
        ELITE:  "text-red-300    border-red-500/30    bg-red-500/10",
    ...
    // Guard against longest === 0 → NaN width
    const progressPct = longest > 0 ? Math.min(100, (current / longest) * 100) : 0
    ...
    return (
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
            <div className="flex flex-col items-center gap-1 w-full sm:w-auto">
                <Flame
                    size={44}
                    className={`${LEVEL_COLOR[level]} ${LEVEL_GLOW[level]} transition-all duration-500`}
                    aria-hidden="true"
                />
                <span className={`text-[32px] font-black leading-none ${LEVEL_COLOR[level]}`}>
                    {current}
                </span>
                <span className="text-[10px] font-mono text-white/25 uppercase tracking-wider">
                    day streak
                </span>
                <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border mt-1 ${LEVEL_PILL[level]}`}>
                    {level}
                </span>
            </div>
            ...
            <div className="flex-1 space-y-3">
                <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl px-3.5 py-2.5">
                    <p className="text-[10px] font-mono uppercase tracking-wider text-white/25 mb-0.5">
                        Longest Streak
                    </p>
                    <div className="flex items-baseline gap-1.5">
                        <span className="text-[24px] font-extrabold text-white">{longest}</span>
                ...
                >
                    <div
                        className={`h-full rounded-full transition-all duration-700 ${
                            level === "ELITE"  ? "bg-gradient-to-r from-red-500    to-orange-400" :
                            level === "HIGH"   ? "bg-gradient-to-r from-orange-500 to-amber-400"  :
                            level === "MEDIUM" ? "bg-gradient-to-r from-amber-500  to-yellow-400" :
                            "bg-white/20"
                        }`}
...
function BadgeCard({ badge }) {
    ...
    return (
        <div
            className={`flex items-center gap-3 p-3 rounded-2xl border ${lm.bg} ${lm.border} ${lm.glow} transition-all duration-300`}
            role="listitem"
        >
            <div
                className={`flex items-center justify-center w-10 h-10 rounded-xl flex-shrink-0 ${lm.bg} border ${lm.border} ${lm.color}`}
                aria-hidden="true"
            >
                {tm.icon}
            </div>
...
function Heatmap({ dailyHistory = [] }) {
    ...
    const cellColor = (count, isToday) => {
        const ring = isToday ? "ring-1 ring-white/30" : ""
        if (count === null) return `bg-white/[0.04] ${ring}`
        if (count >= 30)    return `bg-emerald-400 ${ring}`
        if (count >= 7)     return `bg-violet-500 ${ring}`
        if (count >= 3)     return `bg-violet-500/60 ${ring}`
        return `bg-violet-500/30 ${ring}`
    }

    return (
        <div className="space-y-2">
            {/* Scrollable wrapper prevents layout breakage on narrow screens */}
                ...
                <div className="flex gap-1.5 min-w-max">
                    ...
                    <div className="flex flex-col gap-1 mt-5">
                        {DOW.map((d, i) => (
                            <div
                                key={i}
                                className="h-[14px] flex items-center text-[8px] font-mono text-white/15 w-3"
                            >
                                {d}
                            </div>
                        ))}
                    ...
                    {weeks.map((week, wi) => {
                        ...
                        const showLabel = wi > 0 && wi % 2 === 0
                        return (
                            <div key={wi} className="flex flex-col gap-1">
                                <div className="h-4 flex items-end">
                                    {showLabel && (
                                        <span className="text-[8px] font-mono text-white/15 whitespace-nowrap">
                                ...
                                {week.map(day => (
                                    <div
                                        key={day.key}
                                        title={day.count !== null ? `${day.key} · streak ${day.count}` : day.key}
                                        className={`w-[14px] h-[14px] rounded-sm transition-all duration-300 cursor-default ${cellColor(day.count, day.isToday)}`}
                                        aria-label={day.count !== null
                                            ? `${day.key}, streak ${day.count}`
                                            : `${day.key}, no activity`}
                                    />
                                ))}
                            </div>
...
function Section({ title, subtitle, icon, children }) {
    ...
    return (
        <section
            className="bg-white/[0.025] border border-white/[0.07] rounded-3xl overflow-hidden"
            aria-labelledby={id}
        >
            <div className="flex items-center gap-3 px-5 py-4 border-b border-white/[0.06]">
                <div
                    className="flex items-center justify-center w-8 h-8 rounded-xl bg-white/[0.05] border border-white/[0.08] text-white/40 flex-shrink-0"
                    aria-hidden="true"
                >
                    {icon}
                </div>
                <div>
                    <p id={id} className="text-[13px] font-bold text-white/80">{title}</p>
                    {subtitle && <p className="text-[10px] text-white/25 mt-0.5">{subtitle}</p>}
                </div>
            </div>
            <div className="px-5 py-4">{children}</div>
...
function EmptyBadges() {
    const hints = [
        { label: "60% accuracy", sub: "Bronze", color: "text-amber-600/60"  },
        { label: "75% accuracy", sub: "Silver", color: "text-slate-400/60"  },
        { label: "7-day streak", sub: "Streak", color: "text-violet-400/60" },
    ]
    return (
        <div className="flex flex-col items-center justify-center py-8 gap-3">
            <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-white/[0.04] border border-white/[0.07]">
                <Trophy size={22} className="text-white/15" aria-hidden="true" />
            </div>
            <p className="text-[13px] text-white/30 font-semibold">No badges yet</p>
            <p className="text-[11px] text-white/20 text-center max-w-[200px]">
                Complete habits consistently to unlock accuracy and streak badges.
            </p>
            {/* flex-wrap so hints reflow on mobile instead of a rigid 3-column grid */}
...
export default function AnalyticsPage() {
    ...
    if (error) return (
        <div className="min-h-screen bg-[#08080f] flex items-center justify-center p-6">
            <div
                role="alert"
                className="flex items-center gap-3 bg-red-500/[0.08] border border-red-500/25 rounded-2xl px-5 py-4 max-w-sm w-full"
            >
    ...
    return (
        <div className="min-h-full overflow-x-hidden bg-[#08080f] text-white">
            <div className="pointer-events-none fixed top-0 right-0 w-[500px] h-[500px] rounded-full bg-violet-700/[0.07] blur-[140px] z-0" aria-hidden="true" />
            <div className="pointer-events-none fixed bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-indigo-700/[0.06] blur-[110px] z-0" aria-hidden="true" />
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 sm:px-6 md:px-8 lg:px-10 md:py-10 space-y-5">
                ...
                <header>
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        {!loading && trend !== 0 && (
                            <div
                                className={`flex items-center gap-1 text-[12px] font-bold ${trend > 0 ? "text-emerald-400" : "text-red-400"}`}
                                aria-label={`${trend > 0 ? "Up" : "Down"} ${Math.abs(trend).toFixed(0)}% versus last week`}
                            >
                                {trend > 0
                ...
                {/* Loading — skeleton preserves page shape */}
                {loading ? (
                    <PageSkeleton />
                ) : (
                    <>
                        ...
                        <dl className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                            {topStats.map(s => (
                                <div
                                    key={s.label}
                                    className="bg-white/[0.03] border border-white/[0.07] rounded-2xl px-4 py-3.5"
                                >
                                    <div className="flex items-center gap-1.5 mb-2">
                                        <span aria-hidden="true">{s.icon}</span>
                                        <dt className="text-[9px] font-mono uppercase tracking-wider text-white/30">
                                            {s.label}
                                        </dt>
                                    </div>
                                    <dd className={`text-[24px] font-extrabold leading-none ${s.color}`}>
                                        {s.val}
                                    </dd>
                                    <p className="text-[9px] text-white/20 mt-1">{s.sub}</p>
                                </div>
                        ...
                        <Section title="Accuracy Over Time" subtitle="Weekly or monthly view" icon={<TrendingUp size={14} />}>
                            <div
                                ...
                                className="flex items-center gap-1.5 mb-4 bg-white/[0.04] border border-white/[0.07] rounded-xl p-1 w-fit"
                            >
                                {["weekly", "monthly"].map(t => (
                                    <button
                                        key={t}
                                        ...
                                        aria-selected={tab === t}
                                        onClick={() => setTab(t)}
                                        className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 ${
                                            tab === t
                            ...
                            {chartData.length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center justify-between text-[10px] font-mono text-white/20">
                                        <span>
                                            {tab === "weekly" ? `${chartData.length} weeks` : `${chartData.length} months`}
                                        </span>
                                        <span>
                                            Latest: {(chartData[chartData.length - 1]?.accuracy ?? 0).toFixed(0)}%
                                        </span>
                                    </div>
                                </div>
                            )}
                            ...
                            {/* Arc summary */}
                            <div className="flex flex-col gap-6 mt-5 pt-4 border-t border-white/[0.06] sm:flex-row sm:items-start sm:justify-around">
                                <AccuracyArc pct={stats?.overallAccuracy ?? 0}                         color="#818cf8" size={110} label="Overall"    />
                                <AccuracyArc pct={weeklyData[weeklyData.length - 1]?.accuracy   ?? 0} color="#38bdf8" size={110} label="This Week"  />
                                <AccuracyArc pct={monthlyData[monthlyData.length - 1]?.accuracy ?? 0} color="#34d399" size={110} label="This Month" />
                            </div>
                        ...
                        {/* Badges — single section, conditional body */}
                        <Section
                            title="Badges"
                            subtitle={badges.length > 0 ? `${badges.length} unlocked` : "Keep going to unlock"}
                            icon={<Award size={14} />}
                        >
                            {badges.length > 0 ? (
                                <div
                        ...
                        {breakdown.length > 0 && (
                            <Section
                                ...
                                subtitle={`${latestReport?.month}/${latestReport?.year}`}
                                icon={<Target size={14} />}
                            >
                                <div className="space-y-2.5">
                                    {[...breakdown]
                                        ...
                                        .map(h => {
                                            const total = latestReport?.totalDays ?? 0
                                            const pct = total > 0 ? Math.round((h.count / total) * 100) : 0
                                            return (
                                                <div key={h.id} className="flex items-center gap-3">
                                                    <span className="text-[11px] font-mono text-white/30 w-6 flex-shrink-0">
                                                        {h.id}
                                                    </span>
                                                    <div
                                                        className="flex-1"
                                                    ...
                                                    <span className="text-[11px] font-mono text-white/40 w-16 text-right tabular-nums">
                                                        {h.count}d ({pct}%)
                                                    </span>
                                                </div>
                                            )
